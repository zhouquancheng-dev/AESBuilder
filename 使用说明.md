# AESBuilder - AES 加密 SO 库一键构建工具

## 这是什么

AESBuilder 可以为不同的 Android 项目快速生成独立的 AES 加密/解密 SO 库。

以前每次为新项目生成 SO，需要手动改 6-7 个文件（C 源码、头文件、CMake、Gradle、Java 类），容易出错。现在只需编辑一个 `config.json`，运行一条命令，即可自动完成全部工作。
...
| `jni_class_package` | Java/Kotlin JNI 类的包名 | `"com.example.myapp.utils"` |
...
```json
{
    ...
    "interference_char": "N",
    "so_name": "ZJEncrypt",
    "jni_class_package": "com.zyhd.network_api.utils",
...
> 注意：`app` 模块直接依赖 `lib_module` 源码编译，所以 `checksignature.h` 中的包名和签名哈希必须匹配当前 app 的配置，否则签名校验不通过，加解密会返回 `UNSIGNATURE`。
